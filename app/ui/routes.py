...
def api_move(data: dict = Body(...), db: Session = Depends(get_db)):
    """
    Make a move. Expects: {game_id, move: 'e2e4'}
    ...
    if hasattr(game, 'white') and hasattr(game, 'black'):
        if (game.white.name == 'Human' and game.black.name == 'AI') or (game.white.name == 'AI' and game.black.name == 'Human'):
            mode = 'human-vs-ai'
    if mode == 'human-vs-ai' and not board.is_game_over():
        # AI move (random legal move for now)
        import random
        ai_move = random.choice([m.uci() for m in board.legal_moves])
